...
from .lipid import LIPID_LIBRARY, build_lipid_3d
...
class VesicleBuilder:
    ...
    FORCEFIELD_DIR = Path("data/vesicle/forcefields")
    FORCEFIELD_INCLUDE = "martini_v2.2.itp"
    DEFAULT_OUTPUT_DIR = Path("outputs/vesicle/basic_vesicle")
    DEFAULT_GRO_NAME = "vesicle.gro"
    DEFAULT_TOP_NAME = "topol.top"

    # 这些是 Martini 官方 Martini 2 lipidome 页面给出的脂质 `.itp` 文件名。
    # builder 会把它们写成指向 `data/vesicle/forcefields/` 的相对 include 路径。
    LIPID_ITP_MAP = {
    ...
    def write_outputs(
        ...
        include 顺序为：
        1. `data/vesicle/forcefields/martini_v2.2.itp`
        2. 当前体系实际用到的官方脂质 `.itp`
        3. 当前体系实际用到的蛋白 `.itp`

        若未显式传入路径，则默认输出到：
        - `outputs/vesicle/basic_vesicle/vesicle.gro`
        - `outputs/vesicle/basic_vesicle/topol.top`
        """
        if gro_name is None:
