...
1. 生成默认 45 蛋白三岛输入。
2. 构建基础囊泡。
3. 写出 `outputs/vesicle/basic_vesicle/vesicle.gro` 与 `topol.top`。
...
from scripts.vesicle_sync_frontend import (
...
OUTPUTS_ROOT = Path("outputs/vesicle")
...
def _derive_dataset_name_for_sync(output_root: Path) -> str:
    ...
    if output_root_resolved.parent != outputs_root:
        warning_message = (
            "自动同步只支持 `outputs/vesicle/<dataset_name>/` 这一层级的输出目录；"
...
def build_default_basic_vesicle(
    ...
    protein_dir: str | Path = "data/vesicle/proteins",
    ...
    自动同步时，输出目录必须位于 `outputs/vesicle/<dataset_name>/` 下；
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="输出目录，默认写到 outputs/vesicle/basic_vesicle/。",
    )
    parser.add_argument(
        "--protein-dir",
        default="data/vesicle/proteins",
        help="蛋白模板目录，默认使用 data/vesicle/proteins/。",
    )
    parser.add_argument(
        "--seed",
        type=int,
    ...
    parser.add_argument(
        "--no-sync-frontend",
        action="store_true",
        help="只写出到 outputs/vesicle，不自动同步到 frontend/visualization/vesicle/。",
    )
    return parser
