"""
vesicle_sync_frontend
=====================

把 `outputs/vesicle/<dataset_name>/` 下的一套囊泡产物同步到
`frontend/visualization/vesicle/<dataset_name>/`，并维护前端用的索引清单。

推荐直接运行：
    python scripts/vesicle_sync_frontend.py

默认行为：
1. 从 `outputs/vesicle/basic_vesicle/` 读取 `vesicle.gro` / `topol.top`
...
DEFAULT_SOURCE_DIR = Path("outputs/vesicle/basic_vesicle")
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="源输出目录，默认使用 outputs/vesicle/basic_vesicle/。",
    )
    parser.add_argument(
        "--visualization-root",
