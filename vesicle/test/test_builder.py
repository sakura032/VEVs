...
import vesicle.scripts.build_default_basic_vesicle as build_default_basic_vesicle_module
from vesicle.scripts.build_default_basic_vesicle import build_default_basic_vesicle
from vesicle.scripts.sync_vesicle_to_frontend import sync_vesicle_to_frontend

PROTEIN_DIR = Path("vesicle/data/proteins")
...
def _workspace_tmp(test_name: str) -> Path:
    """把测试临时文件写到 `work/pytest_temp/`，避免污染主产物目录。"""
...
def _patched_outputs_root(monkeypatch: pytest.MonkeyPatch, test_name: str) -> Path:
    """
    把高层入口里的 `OUTPUTS_ROOT` 临时改到 `work/pytest_temp/` 下，
    避免回归测试污染真实 `vesicle/outputs/`。
    """
    outputs_root = _workspace_tmp(test_name) / "vesicle_outputs"
...
def _write_minimal_output_pair(output_dir: Path) -> None:
    ...
    top_text = '#include "vesicle/data/forcefields/martini_v2.2.itp"\n'
...
def test_builder_write_outputs_emits_gro_and_top_files() -> None:
    """
    验证输出层最基本的稳定性：
    - `.gro` 原子数正确；
    ...
    out_dir = _workspace_tmp("vesicle_builder")
...
def test_sync_vesicle_to_frontend_updates_index_and_preserves_multiple_datasets() -> None:
    """
    直接验证通用同步器：
    - 可把两个不同输出目录同步到前端；
    ...
    - 最新同步的数据集排在最前面。
    """
    visualization_root = _workspace_tmp("vesicle_sync_root")
    source_root = _workspace_tmp("vesicle_sync_sources")
...
def test_build_default_basic_vesicle_writes_and_syncs_named_dataset(
    ...
    """
    高层入口应当支持输出到 `vesicle/outputs/<dataset_name>/`，并自动同步到前端同名目录。
    """
    np.random.seed(4)

    outputs_root = _patched_outputs_root(monkeypatch, "pytest_basic_outputs_root")
    output_dir = _outputs_tmp(outputs_root, "pytest_basic_vesicle_entry")
    frontend_root = _workspace_tmp("pytest_basic_vesicle_frontend")
...
def test_build_default_basic_vesicle_warns_and_errors_when_sync_path_is_outside_outputs() -> None:
    """
    自动同步开启时，输出目录若不在 `vesicle/outputs/<dataset_name>/` 下，应先警告再报错。
    """
    output_dir = _workspace_tmp("outside_outputs_entry")
    frontend_root = _workspace_tmp("outside_outputs_frontend")
...
def test_build_default_basic_vesicle_allows_outside_outputs_when_sync_disabled(
    ...
    """
    关闭自动同步时，不再要求输出目录位于 `vesicle/outputs/` 下。
    """
    _patched_outputs_root(monkeypatch, "pytest_no_sync_outputs_root")
    output_dir = _workspace_tmp("outside_outputs_no_sync")
    builder = build_default_basic_vesicle(
        output_dir=output_dir,
        protein_dir=PROTEIN_DIR,
        sync_frontend=False,
